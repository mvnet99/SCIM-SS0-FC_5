...
IF EXISTS (
    ...
    WHERE  id_Customer = @id_Customer AND WorkflowType_Code = 'MEO_DAILY'
)
BEGIN
...
BEGIN
    ...
    VALUES
        (@id_Customer, 'MEO_DAILY', 'TARGET MEO Daily Delivery',
         'Daily MEO file transformation and delivery — batches all events closed since the prior run',
...
USING (
    VALUES
        ...
        (@id_CustomerWorkflowType, 4, 'SFTP', 'Deliver MEO Zip to TARGET SFTP',
         @SftpConfig,
         3, 120, 1),
        (@id_CustomerWorkflowType, 5, 'EMAIL', 'Send MEO Daily Delivery Notification',
         '{"recipients":["ops-team@company.com"],"subject":"TARGET MEO Daily Files Delivered"}',
         3, 30, 1)
...
BEGIN
    ...
    SET    Schedule_Name = 'TARGET MEO Daily 7AM', Cron_Expression = '0 7 * * *',
...
BEGIN
    ...
    VALUES
        (@id_CustomerWorkflowType, 'TARGET MEO Daily 7AM', '0 7 * * *', 'UTC',
         DATEADD(DAY, 1, CAST(CAST(GETUTCDATE() AS DATE) AS DATETIME)) + '07:00:00',
         'ACTIVE', 1, GETUTCDATE(), GETUTCDATE());
    SET @id_Schedule = SCOPE_IDENTITY();
END

SELECT 'Pewo_CustomerWorkflowType' AS [Table], @id_CustomerWorkflowType AS [id], 'MEO_DAILY' AS [Code];
